...
class Model {
  ...
  static _addDefaultAttributes() {
    ...
    if (this._timestampAttributes.created_at) {
      tail[this._timestampAttributes.created_at] = {
        type: DataTypes.DATE,
        allowNull: false,
        _autoGenerated: true
      };
    ...
    if (this._timestampAttributes.updated_at) {
      tail[this._timestampAttributes.updated_at] = {
        type: DataTypes.DATE,
        allowNull: false,
        _autoGenerated: true
      };
   ...
   * @param {Boolean}                 [options.timestamps=true] Adds created_at and updated_at timestamps to the model.
   ...
   * @param {String|Boolean}          [options.created_at] Override the name of the created_at column if a string is provided, or disable it if false. Timestamps must be true. Not affected by underscored setting.
   * @param {String|Boolean}          [options.updated_at] Override the name of the updated_at column if a string is provided, or disable it if false. Timestamps must be true. Not affected by underscored setting.
  ...
  static init(attributes, options) { // testhint options:none
    ...
    if (this.options.timestamps) {
      if (this.options.created_at !== false) {
        this._timestampAttributes.created_at = this.options.created_at || Utils.underscoredIf('created_at', this.options.underscored);
      }
      if (this.options.updated_at !== false) {
        this._timestampAttributes.updated_at = this.options.updated_at || Utils.underscoredIf('updated_at', this.options.underscored);
      }
   ...
   * @param {Boolean}       [options.silent=false] If true, the updated_at timestamp will not be updated.
  ...
  static upsert(values, options) {
    ...
    const created_atAttr = this._timestampAttributes.created_at;
    const updated_atAttr = this._timestampAttributes.updated_at;
    ...
    return Promise.try(() => {
      ...
      // Attach created_at
      if (created_atAttr && !updateValues[created_atAttr]) {
        const field = this.rawAttributes[created_atAttr].field || created_atAttr;
        insertValues[field] = this._getDefaultTimestamp(created_atAttr) || now;
      }
      if (updated_atAttr && !insertValues[updated_atAttr]) {
        const field = this.rawAttributes[updated_atAttr].field || updated_atAttr;
        insertValues[field] = updateValues[field] = this._getDefaultTimestamp(updated_atAttr) || now;
      }
  ...
  static bulkCreate(records, options) {
    ...
    if (options.updateOnDuplicate) {
      // By default, all attributes except 'created_at' can be updated
      let updatableAttributes = _.pull(Object.keys(this.tableAttributes), 'created_at');
    ...
    return Promise.try(() => {
      ...
      for (const instance of instances) {
        ...
        // set created_at/updated_at attributes
        if (created_atAttr && !values[created_atAttr]) {
          values[created_atAttr] = now;
          options.fields.indexOf(created_atAttr) === -1 && options.fields.push(created_atAttr);
        }
        if (updated_atAttr && !values[updated_atAttr]) {
          values[updated_atAttr] = now;
          options.fields.indexOf(updated_atAttr) === -1 && options.fields.push(updated_atAttr);
        }
  ...
  static update(values, options) {
    ...
    if (options.fields && options.fields instanceof Array) {
      ...
      if (updated_atAttr && options.fields.indexOf(updated_atAttr) === -1) {
        options.fields.push(updated_atAttr);
      }
    }

    if (this._timestampAttributes.updated_at && !options.silent) {
      values[this._timestampAttributes.updated_at] = this._getDefaultTimestamp(this._timestampAttributes.updated_at) || Utils.now(this.sequelize.options.dialect);
    }
    ...
    return Promise.try(() => {
      ...
      if (options.validate) {
        ...
        build.set(this._timestampAttributes.updated_at, values[this._timestampAttributes.updated_at], { raw: true });
  ...
  static increment(fields, options) {
    ...
    const updated_atAttribute = this.rawAttributes[updated_atAttr];
    ...
    if (!options.silent && updated_atAttr && !values[updated_atAttr]) {
      options.attributes[updated_atAttribute.field || updated_atAttr] = this._getDefaultTimestamp(updated_atAttr) || Utils.now(this.sequelize.options.dialect);
    }
    if (versionAttr) {
  ...
  */
  ...
  _initValues(values, options) {
    ...
    if (options.isNewRecord) {
      ...
      if (this.constructor._timestampAttributes.created_at && defaults[this.constructor._timestampAttributes.created_at]) {
        this.dataValues[this.constructor._timestampAttributes.created_at] = Utils.toDefaultValue(defaults[this.constructor._timestampAttributes.created_at], this.sequelize.options.dialect);
        delete defaults[this.constructor._timestampAttributes.created_at];
      }

      if (this.constructor._timestampAttributes.updated_at && defaults[this.constructor._timestampAttributes.updated_at]) {
        this.dataValues[this.constructor._timestampAttributes.updated_at] = Utils.toDefaultValue(defaults[this.constructor._timestampAttributes.updated_at], this.sequelize.options.dialect);
        delete defaults[this.constructor._timestampAttributes.updated_at];
      }
  ...
  save(options) {
    ...
    const created_atAttr = this.constructor._timestampAttributes.created_at;
    const versionAttr = this.constructor._versionAttribute;
    const hook = this.isNewRecord ? 'Create' : 'Update';
    const wasNewRecord = this.isNewRecord;
    const now = Utils.now(this.sequelize.options.dialect);
    let updated_atAttr = this.constructor._timestampAttributes.updated_at;

    if (updated_atAttr && options.fields.length >= 1 && options.fields.indexOf(updated_atAttr) === -1) {
      options.fields.push(updated_atAttr);
    }
    ...
    if (options.silent === true && !(this.isNewRecord && this.get(updated_atAttr, {raw: true}))) {
      // UpdateAtAttr might have been added as a result of Object.keys(Model.attributes). In that case we have to remove it again
      _.remove(options.fields, val => val === updated_atAttr);
      updated_atAttr = false;
    }

    if (this.isNewRecord === true) {
      if (created_atAttr && options.fields.indexOf(created_atAttr) === -1) {
        options.fields.push(created_atAttr);
      }
    ...
    if (updated_atAttr && !options.silent && options.fields.indexOf(updated_atAttr) !== -1) {
      this.dataValues[updated_atAttr] = this.constructor._getDefaultTimestamp(updated_atAttr) || now;
    }

    if (this.isNewRecord && created_atAttr && !this.dataValues[created_atAttr]) {
      this.dataValues[created_atAttr] = this.constructor._getDefaultTimestamp(created_atAttr) || now;
    }
    ...
    return Promise.try(() => {
      ...
      if (options.hooks) {
        ...
        if (updated_atAttr && options.fields.indexOf(updated_atAttr) !== -1) {
          ignoreChanged = _.without(ignoreChanged, updated_atAttr);
        }
